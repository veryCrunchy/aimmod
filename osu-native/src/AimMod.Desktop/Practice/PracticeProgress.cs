...
public static class PracticeProgressTracker
{
    ...
    public static bool SameSource(LocalReplay r, PracticeTracking t) => Stable(r) == t.Stable && ScoreMods.Configuration(r) == t.SourceMods &&
        ((t.OnlineBeatmapId > 0 && r.OnlineBeatmapId == t.OnlineBeatmapId) || (t.SourceHash.Length > 0 && string.Equals(r.BeatmapHash, t.SourceHash, StringComparison.OrdinalIgnoreCase)) || (r.IsLocallyStored && r.BeatmapId == t.SourceBeatmapId));
    private static bool valid(LocalReplay r) => ScoreMods.IsManualPlay(r) && double.IsFinite(r.Accuracy) && r.Accuracy is >= 0 and <= 1 && r.MissCount >= 0 && r.PlayedAt <= DateTimeOffset.UtcNow;
    public static PracticeAttempt Snapshot(LocalReplay r, string name, bool original) => new(r.ScoreId, r.OnlineScoreId, r.PlayedAt, r.Accuracy, r.MissCount, r.Passed,
        (Stable(r) ? "stable" : "lazer") + " / " + ScoreMods.Display(r) + " / " + ScoreMods.Configuration(r), name, original);
    ...
    public static PracticeProgress Reconcile(SavedPracticeMap map, PracticeProgress previous, IEnumerable<LocalReplay> history, int accountId)
    {
        ...
        var attempts = previous.Attempts.ToDictionary(a => a.ScoreId);
        foreach (var r in history.Where(r => valid(r) && SamePlayer(r,t.Player) && r.PlayedAt >= map.CreatedAt))
        {
            ...
            if (difficulty is not null) attempts[r.ScoreId] = Snapshot(r,difficulty.Name,false);
            else if (SameSource(r,t)) attempts[r.ScoreId] = Snapshot(r,"Original map",true);
        }
    ...
    {
        var all = attempts.OrderBy(a => a.PlayedAt).ToArray(); var done = all.Where(a => a.Passed).ToArray();
        if (all.Length == 0) return "Not practised yet";
        string text = $"{all.Length} recorded attempts / {done.Length} completed";
