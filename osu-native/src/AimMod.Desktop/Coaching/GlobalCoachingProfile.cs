...
public static class GlobalCoachingProfileBuilder
{
        ...
        IReadOnlyDictionary<Guid, ReplayAnalysisResult> analyses)
    {
        ...
        GlobalMissReasonShare[] reasons = misses.Where(item => item.MissAnalysis is { Reason: not ReplayMissReason.Unknown })
                                                .GroupBy(item => item.MissAnalysis!.Reason)
                                                .Select(group =>
                                                {
                                                    HashSet<Guid> scoreIds = exact.Where(item => item.Analysis!.Judgements.Any(judgement =>
                                                        judgement.MissAnalysis?.Reason == group.Key))
                                                                                  .Select(item => item.Run.ScoreId)
                                                                                  .ToHashSet();
                                                    int mapCount = exact.Where(item => scoreIds.Contains(item.Run.ScoreId))
                                                                        .Select(item => mapKey(item.Run))
                                                                        .Distinct(StringComparer.Ordinal)
                                                                        .Count();
                                                    return new GlobalMissReasonShare(
                                                        group.Key,
                                                        group.Count(),
                                                        misses.Length == 0 ? 0 : (double)group.Count() / misses.Length,
                                                        scoreIds.Count,
                                                        mapCount);
                                                })
        ...
        GlobalCoachingPriority[] priorities = buildPriorities(coverage, reasons, recurring, timing, cursorDistances);
        ...
        IReadOnlyList<GlobalMissReasonShare> reasons)
    {
        ...
        {
            ReplayMissReason[] mapReasons = map.SelectMany(item => item.Analysis!.Judgements)
                                                .Where(isMiss)
                                                .Select(item => item.MissAnalysis?.Reason ?? ReplayMissReason.Unknown)
                                                .Where(reason => reason != ReplayMissReason.Unknown)
                                                .ToArray();
            if (mapReasons.Length < 2)
                continue;

            IGrouping<ReplayMissReason, ReplayMissReason> dominant = mapReasons.GroupBy(reason => reason)
            ...
            weaknesses.Add(new GlobalRecurringWeakness(
                $"map:{map.Key}:{dominant.Key}",
                $"Repeated on {latest.Run.Title}",
                $"{ReplayMissInsightPresenter.Label(dominant.Key)} appeared {dominant.Count():N0} times across {map.Count():N0} analysed attempts on [{latest.Run.Difficulty}].",
                dominant.Count(),
                map.Count(),
                1));
        }

        return weaknesses.OrderByDescending(item => item.MapCount > 1)
    ...
    private static GlobalCoachingPriority[] buildPriorities(
        ...
        IReadOnlyList<double> cursorDistances)
    {
        var priorities = new List<GlobalCoachingPriority>();
        GlobalMissReasonShare? dominant = reasons.FirstOrDefault();
        ...
        if (cursorDistances.Count >= 10)
        {
            double p90 = percentile(cursorDistances, 0.9);
            priorities.Add(new GlobalCoachingPriority(
                "Aim precision baseline",
                $"90% of measured successful hits land within {p90:0.0} playfield units. Use this as a personal baseline when comparing future sessions.",
                $"p90 {p90:0.0} px",
                coverage.Confidence));
        }
        ...
        return priorities.Take(4).ToArray();
    }
